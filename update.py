...
def parse_station_data():
    ...
    with open('CT-GTFS/stops.txt', 'r', encoding=None) as stopsFile:
        ...
        for row in stopsReader:
            ...
            stop_name = ' '.join(
                i for i in row[stop_name_x].split() if i not in extra)
...
def parse_gtfs_schedule_data(stops):
    ...
    with open('CT-GTFS/stop_times.txt', 'r') as timesFile:
        ...
        sortedLines = sorted(
            timesReader, key=lambda row: int(
                row[departure_x].replace(
                    ':', '')))
        for row in sortedLines:
            ...
            if (trip_id not in _times[schedule][direction]):
                _times[schedule][direction][trip_id] = [
                    None] * len(stops[direction])
            _times[schedule][direction][trip_id][stops[direction].index(
                stop_id)] = departure
    return _times
