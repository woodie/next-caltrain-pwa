const prefs = new LocalStorage(caltrainServiceData.southStops)
const service = new CaltrainService()
let app = false
let android = false
let kaiWeb1 = false
let kaiWeb2 = false
...
class NextCaltrain {
  ...
  static bumpKeypadHint () {
    ...
    if (Array.isArray(hints[hintIndex][1])) {
      ...
      for (let i = 1; i < 15; i++) {
        const key = (i < 10) ? i : ['l', 'r', 'c', 'o', 'h'][i - 10]
        const cls = hints[hintIndex][1].indexOf(key) === -1 ? 'default' : 'selected'
  ...
  static moveMenuSelection () {
    const menuOptions = document.getElementById('menu-list').getElementsByTagName('li')
  ...
  static setTheTime () {
    const ourTime = new GoodTimes()
    const partTime = ourTime.partTime()
  ...
  static setCountdown (minutes) {
    const downTime = new GoodTimes()
    const blurb = downTime.countdown(minutes)
    document.getElementById('blurb-grid').innerHTML = blurb
    document.getElementById('blurb-hero').innerHTML = blurb
    if (blurb !== '') {
      const refresh = blurb.endsWith('sec') ? 1000 : (60 - downTime.seconds) * 1000
  ...
  static loadTrip (train) {
    ...
    const trip = new CaltrainTrip(train, schedule.label())
    const lines = []
    for (let i = 0; i < trip.times.length; i++) {
      const stop = trip.times[i]
      const spacer = (i === 0) ? '' : '|'
      const fullTime = GoodTimes.fullTime(stop[1])
      const filler = fullTime.length > 6 ? '' : '0'
      const style = (goodTime.inThePast(stop[1])) ? 'message-departed' : 'message-arriving'
      const target = (prefs.origin === stop[0] || prefs.destin === stop[0]) ? 'target' : ''
  ...
  static loadSchedule () {
    ...
    // Load the schedule
    const routes = service.routes(prefs.origin, prefs.destin, schedule.label())
    let minutes = 0
    if (offset === null) {
    ...
    for (let i = 0; i < 6; i++) {
      const tripCardElement = document.getElementById(`trip${i}`)
      const n = (offset + i > routes.length - 1) ? offset + i - routes.length : offset + i
      ...
      minutes = route[1]
      const originTime = GoodTimes.partTime(minutes)
      const destinTime = GoodTimes.partTime(route[2])
      const card = `<div class="train-number">#${route[0]}</div>
      ...
      if (i === 0) {
        const tripTime = `<span class="train-hero">#${route[0]}</span>
  ...
  static displayScreen (target) {
    ...
    for (let i = 0; i < screens.length; i++) {
      const display = (target === screens[i]) ? 'flex' : 'none'
  ...
  static press (code) {
    ...
    if (code === ESC) {
      ...
      const confirmation = ['Save', (prefs.flipped ? prefs.destin : prefs.origin), 'as morning and',
      ...
      if (code === OK) {
        const menuOptions = document.getElementById('menu-list').getElementsByTagName('li')
        const action = menuOptions[menuIndex].getAttribute('value')
        menuIndex = 0
